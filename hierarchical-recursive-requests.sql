...
-- А мы, к примеру, хотим сделать отступ,
-- и чем ниже ранг, тем больше отступ.

-- Функция LPAD просто приклеивает к какому-то тексту какие-то символы слева, пока он не станет ожидаемой длины.

SELECT s.id,
    LPAD (' ', (level - 1) * 5, ' ') || s.fio FIO, -- level показывает уровень вложенности, то есть сколько раз табличка сама с собой соединяется
    s.statusname,
    s.Bossid
    FROM Sotrudniki s
    CONNECT BY prior ID = BossID
...
-- А теперь попробуем наоборот, зная сотрудника, попробуем вывести его босса.
SELECT
    s.id,
    s.FIO,
    s.statusname,
    s.Bossid
    FROM Sotrudniki s
    CONNECT BY ID = prior BossID
    START WITH BOSSID IS NULL

-- А теперь возьмём энного сотрудника, например инженера, и узнаем кто у него руководитель
-- Например начнем с человека, у которого FIO 'Васи В.В.'
-- (Можно начать и с айди, но мы использовали FIO)
SELECT
    s.id,
    s.FIO,
    s.statusname,
    ...
    FROM Sotrudniki s
    CONNECT BY ID = prior BossID
    START WITH FIO = 'Формат Константин Борисович' -- Ну или 'Васи В.В.'...
